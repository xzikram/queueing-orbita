...
export default function CdcPage() {
  ...
  const action = async (visitId: string, endpoint: string) => {
    ...
    try { await api.post(`/cdc/${visitId}/${endpoint}`); await loadQueue(); }
          ...
          <div className={styles.queueList}>
            {waiting.length === 0 ? <div className={styles.empty}>Tidak ada</div> : waiting.map((v: any) => (
              <div key={v.id} className={styles.queueCard}>
                ...
                <button className="btn btn-success btn-sm" style={{ width: '100%', marginTop: 8 }} onClick={() => action(v.id, 'start')} disabled={actionLoading === v.id}>▶️ Mulai CDC</button>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className={styles.queueList}>
            {serving.length === 0 ? <div className={styles.empty}>Tidak ada</div> : serving.map((v: any) => (
              <div key={v.id} className={`${styles.queueCard} ${styles.activeCard}`}>
                ...
                <button className="btn btn-primary btn-sm" style={{ width: '100%', marginTop: 8 }} onClick={() => action(v.id, 'finish')} disabled={actionLoading === v.id}>✅ Selesai → Kasir</button>
              </div>
            ))}
          </div>
        </div>
