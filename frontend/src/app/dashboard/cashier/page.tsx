    ...
    <div className={styles.unitPage}>
      ...
      <div className={styles.columns}>
        ...
        <div className={`glass-card ${styles.column}`}>
          ...
          <div className={styles.queueList}>
            {active.length === 0 ? <div className={styles.empty}>Tidak ada</div> : (
              active.map((v: any) => {
                const s = v.journeySessions?.[0];
                    ...
                    <div className={styles.actionBtns}>
                      {(s?.status === 'CALLED' || s?.status === 'SERVING') && (
                        <>
                          ...
                          <button className="btn btn-warning btn-sm" onClick={() => action(v.id, 'call', { counterId: selectedCounter })} disabled={actionLoading === v.id || !selectedCounter || !isMyTicket}>🔁 Panggil Ulang</button>
                        </>
                      )}
